...
if $mysql_values['root_password'] {
  class { 'mysql::server':
    ...
    override_options => { 'mysqld' => { 'max_connections' => '1024' ,'datadir' => '/var/vagrantmysql','socket' => '/tmp/mysql.sock'}, 'client' => {'socket' => '/tmp/mysql.sock'}, 'mysqld_safe' => {'socket' => '/tmp/mysql.sock'} }
  }
